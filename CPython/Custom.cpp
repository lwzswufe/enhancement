...
// 定义Python对象函数
static PyObject * Custom_change_name(CustomObject *self, PyObject *args, PyObject *kwds)
{   // Py_UNUSED 这个可用于函数定义中未使用的参数以隐藏编译器警告
    static char *kwlist[] = {member_name_first, 
                             member_name_last,
                             NULL};
    PyObject *first{nullptr}, *last{nullptr}, *tmp{nullptr};
    // printf("self:%p args:%p kwds:%p\n", self, args, kwds);
    // 解析构造函数参数
    int ok = PyArg_ParseTuple(args, "OO", &first, &last);
    if (!ok)
    {
        printf("error in parse args\n");
        Py_RETURN_NONE;
    }
    // int ok = PyArg_ParseTupleAndKeywords(args, kwds, "|OO", kwlist, &first, &last);
    // if (!ok)
    // {   
    //     printf("error in parse args\n");
    //     Py_RETURN_NONE;
    // }
    // printf("first:%p %p last:%p %p\n", self->first, first, self->last, last);
    if (first)                  // 若原first值不为空
    {
        tmp = self->first;
        Py_INCREF(first);       // 增加 新赋值变量first的 引用计数
        self->first = first;
        printf("change first name: %s->%s\n", PyUnicode_AsUTF8(tmp), PyUnicode_AsUTF8(first));
        Py_XDECREF(tmp);        // 减少 原变量的  引用计数
    }
    if (last)                   // 若原last值不为空
    {
        ...
        Py_INCREF(last);        // 增加 新赋值变量last的 引用计数
        self->last = last;
        printf("change last name: %s->%s\n", PyUnicode_AsUTF8(tmp), PyUnicode_AsUTF8(last));
        Py_XDECREF(tmp);        // 减少 原变量的  引用计数
    }
    printf("change name exit\n");
    Py_RETURN_NONE;
}
...
PyMODINIT_FUNC PyInit_custom(void)
{
    ...
    Custom_methods[1] = {"change_name", (PyCFunction) Custom_change_name, METH_VARARGS, "Change the name, first name and last name"};
    Custom_methods[2] = {NULL};
    // 对模块变量列表进行初始化
