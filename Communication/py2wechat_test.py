...
from Communication import wechat_auto_reply
...
class send_message_to_wechat(object):
    def __init__(self, time_interval=3,
        ...
        self.wechat_id = dict()
        self.reply = wechat_auto_reply.auto_reply()
        ...
        def group_text_reply(msg):
            return self.reply.reply_signal(msg, wechat_class=self)

        @itchat.msg_register(isGroupChat=True, msgType=TEXT)   # 微信群
        def group_text_reply(msg):
            return self.reply.reply_group(msg, wechat_class=self)
    ...
    def send_wechat_file(self, mandatory_order=False):
        ...
        if mandatory_order:
            pass
        elif clock[3] * 100 + clock[4] < 1133 or self.is_send_file or clock[3] * 100 + clock[4] > 1559:
            return
