...
class ID3_tree(object):
    ...
    def load_dataset(self, fname, labels):
        record_list = []
        f = open(fname, 'rb')
        content = f.read()
    ...
    def build_tree(self, label, dataset):
        ...
        # 算法核心
        best_feat = self.get_best_feat()
        best_feat_label = label[best_feat]
        tree = {best_feat_label:{}}
        del label[best_feat]
        # 抽取最优特征值向量
        unique_vals = set([data[best_feat] for data in self.dataset])
        for value in unique_vals:
            sub_labels = label[:]
            split_data = self.split_dataset(self.dataset, best_feat, value)
    ...
    def get_best_feat(self, dataset):
        ...
        base_entropy = self.compute_entropy()
